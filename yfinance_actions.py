...
my_test_string: str = "pull website, state, day high, and day low for apple, microsoft, and alphabet"


# functions that edit/clean user inputs/lists

# removes endings like "inc." and "company" to match likely user inputs
def remove_ending(full_company_name: str) -> str:
    endings: list[str] = ['incorporated', 'corporation', 'inc', 'company', 'limited', 'incorporated', 'plc', 'holdings',
                          'holding',
...
# functions that fetch names or time
...
# functions that store/ retrieve data
...
def fetch_info(ticker: str, financial_term: str):
    ...
    if highest_probability >= threshold:
        ...
        try:
            print(
                f"{convert_from_pascal(requested_financial_info)}: {yf.Ticker(ticker).info[f'{requested_financial_info}']}")
    ...
    # checking ticker object attributes for the term
        ...
        requested_financial_info: str = yfinance_keys[idx]
        print(
            f"{convert_from_pascal(requested_financial_info)}: {yfinance_properties_dict(ticker).get(requested_financial_info)}")


# functions that convert english into finance bot speak:
...
def validate_company_name(user_string: str, companies: list[str], whole_dict: dict[str], tickers: list[str]) -> str:
    ...
    for company in companies:
        ...
        if similar(clean_user, clean_company) >= 0.85:
            ...
            request_company_name_replies(company)
            return list(whole_dict.keys())[list(whole_dict.values()).index(company)]
        # REMOVING THIS FOR NOW TO SEE SEARCH - you can check if string is all upper and run that for ticker instead
...
def fetch_finance_term_per_company(user_string: str, user_terms):
    ...
    for word in user_words:
        name_exists = validate_company_name(word, ticker_company_pairs.values(), ticker_company_pairs,
                                            ticker_company_pairs.keys())
        if name_exists:
...
# CONDUCTING TESTS BELOW

def asked_bot_term_company(user_input: str):
    fetch_finance_term_per_company(user_input, check_existence_of_financial_term(user_input))
